...
bool NimBLEClient::connect(const NimBLEAddress &address, bool deleteAttibutes) {
    ...
    // Wait for the connect timeout time +500ms second for the connection to complete
    if(ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(m_connectTimeout + 500)) == pdFALSE) {
        m_pTaskData = nullptr;
...
int NimBLEClient::disconnect(uint8_t reason) {
    ...
    if(isConnected()) {
        ...
        // If this client connection ID does not match the address
        // found by the stack, we need to reset the host.
        ble_addr_t peerAddr_t;
        ...
        if(ble_gap_conn_find_by_addr(&peerAddr_t, &desc) == 0){
            if(desc.conn_handle != m_conn_id) {
                NIMBLE_LOGC(LOG_TAG, "Connection id & address mismatch - resetting stack");
                ble_hs_sched_reset(BLE_HS_ECONTROLLER);
        ...
        // not inform the stack when disconnection is complete. This is a common
        // error in older esp-idf versions.
        ...
        // Make sure the timer started before terminating the connection.
        // Otherwise subsequent calls to this function may reset the timer,
        // or the disconnect event may come and stop the timer before it is started.
        while(ble_npl_callout_is_active(&m_dcTimer) == 0) {
            taskYIELD();
        }
